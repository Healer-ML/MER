...
if __name__ == "__main__":
    ...
    print(f"nubmber: {len(subs)}, sub: {subs}")
    ...
    for label_name, label_index in label_map.items():
        ...
        print(f"label {label_index}: {count} sample")
    ...
    for n_subname in subs:
        ...
        print('trains', len(train_indices))
        print('tests', len(test_indices))
        ...
        for epoch in range(1, args.epochs + 1):
            if args.train:
                model.train()
                running_loss, num_train_correct, num_train_examples = 0.0, 0, 0
                for img, gt_label in train_loader:
                    img, gt_label = img.to(device), gt_label.to(device)
                    optimizer.zero_grad()
                    pre_emotion, fea = model(img)
                    loss = loss_fn(pre_emotion, gt_label)
                    loss.backward()
                    optimizer.step()
                    running_loss += loss.item() * img.size(0)
                    preds = torch.max(pre_emotion, 1)[1]
                    num_train_correct += (preds == gt_label).sum().item()
                    num_train_examples += gt_label.size(0)
            ...
            with torch.no_grad():
                for img, gt_label in test_loader:
                    img, gt_label = img.to(device), gt_label.to(device)
                    # pre_emotion, gf,tl,tr,bl,br = model(img)
        ...
        if val_acc > best_accuracy_for_each_subject:
            ...
            best_each_subject_pred = current_pred.copy()
            if args.train:
                torch.save(model.state_dict(), weight_path)
                print(f"Subject {n_subname} | Epoch [{epoch}/{args.epochs}] | Saved best model: {weight_path}")
        if val_acc == 1:
            break
        total_pred.extend(current_pred)
        total_gt.extend(current_truth)
        accuracydict = {'pred': best_each_subject_pred, 'truth': current_truth}
        ...
        UF1, UAR, F1_score = recognition_evaluation(current_truth, best_each_subject_pred, show=True,
